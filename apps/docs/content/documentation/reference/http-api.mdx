...
`$pageview`. The API re-sanitizes payloads before forwarding them to Durabull's collector.

`POSTHOG_KEY` only configures an instance owner's own PostHog destination. It does not disable
Durabull anonymous telemetry, and Durabull does not provide a product-level telemetry opt-out.
