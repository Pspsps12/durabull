...
| `DURABULL_REDIS_URL_STARTUP_MIGRATION` | Temporary | Runs one-time startup migration to encrypt legacy plaintext connection URLs (`true` by default) |
| `DURABULL_ENFORCE_REDIS_URL_ENCRYPTION` | Optional | Strict plaintext enforcement toggle used when startup migration is disabled (`true`/`false`). Defaults to `true` in production |
